...
export const StudentSidebar: React.FC<StudentSidebarProps> = ({
  ...
  const practiceTools = [
    {
      ...
      icon: AlertTriangle,
      count: mistakesCount,
      countColor: 'bg-rose-100 text-rose-700 border-rose-200',
    },
    {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-50 bg-white border-r border-slate-200/90 flex flex-col transition-all duration-300 ease-in-out select-none shadow-sm lg:static lg:translate-x-0',
      ...
      >
        ...
        <div
          className={cn(
            'h-16 flex items-center border-b border-slate-100 px-4 transition-all duration-300',
        ...
        >
          ...
          >
            <div className="w-10 h-10 rounded-xl bg-blue-600 flex items-center justify-center shadow-md shadow-blue-500/20 group-hover:scale-105 transition-transform shrink-0">
              <img
            ...
            <div className={cn('flex flex-col', isCollapsed && 'lg:hidden')}>
              <span className="font-black text-lg text-slate-900 tracking-tight leading-tight flex items-center">
        ...
        <div className={cn('px-3 pt-3 pb-1', isCollapsed && 'lg:hidden')}>
          <div className="relative">
            <button
              type="button"
              onClick={() => setExamDropdownOpen(!examDropdownOpen)}
              className="w-full flex items-center justify-between p-2.5 rounded-xl bg-slate-50 hover:bg-slate-100/80 border border-slate-200/80 transition-all text-left group"
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="w-7 h-7 rounded-lg bg-blue-100 text-blue-700 flex items-center justify-center shrink-0 font-bold text-xs border border-blue-200/60">
                  WB
                </div>
        ...
        <div className="flex-1 py-3 px-3 space-y-6 overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-slate-200">
              ...
              {practiceTools.map((tool) => {
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      'flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold text-slate-600 hover:text-slate-900 hover:bg-slate-100/80 transition-all group',
                      isCollapsed && 'lg:justify-center lg:px-2'
                    )}
                  >
          ...
          {/* Pro Pass Card Banner */}
          {/* Expanded Card (Mobile or Desktop Expanded) */}
          <div className={cn('pt-2', isCollapsed && 'lg:hidden')}>
            {isPro ? (
              <div className="p-3.5 rounded-2xl bg-gradient-to-br from-slate-900 to-indigo-950 text-white border border-indigo-900/40 shadow-sm relative overflow-hidden">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    <Crown className="w-4 h-4 text-amber-400 fill-amber-400" />
                    <span className="text-xs font-extrabold tracking-wide text-amber-300 uppercase">
                      Pro Pass Active
                    </span>
                  </div>
                ...
                <p className="text-[11px] text-slate-300 mt-1.5 font-medium">
                  All Premium Mock Tests & Full Analytics Unlocked
                </p>
                <Link
                  to="/subscription"
                  onClick={onClose}
                  className="inline-flex items-center gap-1 text-[11px] font-bold text-amber-400 hover:text-amber-300 mt-2.5 transition-colors"
                >
                  Manage Pass <ArrowRight className="w-3 h-3" />
                </Link>
              </div>
            ) : (
              <div className="p-3.5 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 text-white shadow-md shadow-blue-600/20 relative overflow-hidden">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-blue-100 font-medium">₹299 / 365 Days</p>
                  </div>
                </div>
                <p className="text-[11px] text-blue-50 mt-2 leading-snug font-medium">
                  Unlock 100+ Full Mocks, Chapter Tests & PYQs.
                </p>
                <button
                  onClick={() => {
                    onClose();
                  ...
                  className="w-full mt-3 py-2 px-3 rounded-xl bg-white text-blue-700 font-bold text-xs hover:bg-blue-50 transition-colors shadow-sm flex items-center justify-center gap-1.5"
                >
          ...
          {isCollapsed && (
            <div className="hidden lg:flex justify-center pt-2">
              <button
                ...
                title={isPro ? 'Pro Pass Active' : 'Upgrade to Pro Pass (₹299/365d)'}
                className={cn(
              ...
              >
                <Crown className={cn('w-5 h-5', isPro && 'fill-slate-950')} />
              </button>
            </div>
          )}
        </div>
