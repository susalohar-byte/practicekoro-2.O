...
const StatCard: React.FC<StatCardProps> = ({ icon, label, value, subtitle, gradient, iconBg }) => (
  <div className={`relative overflow-hidden rounded-2xl p-5 ${gradient} border border-white/10 dark:border-white/5 shadow-lg shadow-black/5 dark:shadow-black/20`}>
...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  const config: Record<string, { bg: string; text: string; dot: string }> = {
    active: { bg: 'bg-emerald-500/10 dark:bg-emerald-500/15', text: 'text-emerald-600 dark:text-emerald-400', dot: 'bg-emerald-500' },
    expired: { bg: 'bg-rose-500/10 dark:bg-rose-500/15', text: 'text-rose-600 dark:text-rose-400', dot: 'bg-rose-500' },
    cancelled: { bg: 'bg-slate-500/10 dark:bg-slate-500/15', text: 'text-slate-500 dark:text-slate-400', dot: 'bg-slate-400' },
    completed: { bg: 'bg-emerald-500/10 dark:bg-emerald-500/15', text: 'text-emerald-600 dark:text-emerald-400', dot: 'bg-emerald-500' },
    pending: { bg: 'bg-amber-500/10 dark:bg-amber-500/15', text: 'text-amber-600 dark:text-amber-400', dot: 'bg-amber-500' },
    failed: { bg: 'bg-rose-500/10 dark:bg-rose-500/15', text: 'text-rose-600 dark:text-rose-400', dot: 'bg-rose-500' },
  };
...
// ─── Empty State ────────────────────────────────────────────────
const EmptyState: React.FC<{ icon: React.ReactNode; title: string; subtitle: string; isLoading?: boolean }> = ({ icon, title, subtitle, isLoading }) => (
...
export const AdminSubscriptions: React.FC = () => {
  ...
  // ─── Tab Config ─────────────────────────────────────────────
  const tabs = [
    { key: 'subscriptions' as const, label: 'Active Subs', icon: CreditCard, count: subscriptions.length },
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'subscriptions' && (
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <SearchBar value={subSearch} onChange={setSubSearch} placeholder="Search by name or email..." />
            <div className="flex gap-2">
              ...
              <button
                ...
                title="Refresh"
              >
                <RefreshCw className={`w-4 h-4 ${subLoading ? 'animate-spin text-indigo-500' : ''}`} />
              </button>
            </div>
          </div>
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="bg-slate-50/80 dark:bg-slate-800/30 border-b border-slate-200 dark:border-slate-700/50">
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Student</th>
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Plan</th>
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Status</th>
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Period</th>
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Days Left</th>
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Payment Ref</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                  {subscriptions.length === 0 ? (
                    ...
                    subscriptions.map((sub) => (
                      <tr key={sub.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors group">
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            <Avatar name={sub.studentName} isPro={sub.status === 'active'} />
                            <div className="min-w-0">
                              <div className="font-semibold text-slate-900 dark:text-white text-sm">{sub.studentName}</div>
                              <div className="text-xs text-slate-500 dark:text-slate-400 truncate">{sub.studentEmail}</div>
                              {sub.studentPhone && (
                                <div className="text-[11px] text-slate-400 dark:text-slate-500">{sub.studentPhone}</div>
                              )}
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-5 py-4">
                          <div className="text-xs text-slate-600 dark:text-slate-300">
                            {new Date(sub.startsAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
                          </div>
                          <div className="text-[11px] text-slate-400 dark:text-slate-500">
                            to {new Date(sub.expiresAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                          </div>
                        </td>
                        <td className="px-5 py-4">
                          {sub.status === 'active' ? (
                            <div className="flex items-center gap-1.5">
                              <div className="w-16 h-1.5 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden">
                                <div
                                  ...
                                  style={{ width: `${Math.min(100, (sub.daysRemaining / 365) * 100)}%` }}
                                />
                              </div>
                              <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400">{sub.daysRemaining}d</span>
                            </div>
                          ) : (
      ...
      {activeTab === 'students' && (
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <SearchBar value={studentSearch} onChange={setStudentSearch} placeholder="Search student by name or email..." />
            <div className="flex gap-2">
              <div className="relative">
                <select
                  value={studentFilter}
              ...
              <button
                ...
                title="Refresh"
              >
                <RefreshCw className={`w-4 h-4 ${studentLoading ? 'animate-spin text-indigo-500' : ''}`} />
              </button>
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="bg-slate-50/80 dark:bg-slate-800/30 border-b border-slate-200 dark:border-slate-700/50">
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Student</th>
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Membership</th>
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Plan</th>
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Expiry</th>
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Registered</th>
                      ...
                      <tr key={st.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors">
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <div className="font-semibold text-slate-900 dark:text-white text-sm truncate">{st.fullName}</div>
                              <div className="text-xs text-slate-500 dark:text-slate-400 truncate">{st.email}</div>
                              {st.phone && <div className="text-[11px] text-slate-400">{st.phone}</div>}
                            </div>
                          </div>
                        </td>
                        <td className="px-5 py-4">
      ...
      {activeTab === 'payments' && (
        <div className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <SearchBar value={paySearch} onChange={setPaySearch} placeholder="Search by student name, order ID, or payment ID..." />
            <div className="flex gap-2">
              <div className="relative">
                <select
                  value={payFilter}
              ...
              <button
                ...
                title="Refresh"
              >
                <RefreshCw className={`w-4 h-4 ${payLoading ? 'animate-spin text-indigo-500' : ''}`} />
              </button>
            </div>
          </div>
              ...
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="bg-slate-50/80 dark:bg-slate-800/30 border-b border-slate-200 dark:border-slate-700/50">
                    ...
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Student</th>
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Plan</th>
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Amount</th>
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Gateway</th>
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Transaction</th>
                    <th className="px-5 py-3.5 text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Status</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="text-xs text-slate-500 dark:text-slate-400 truncate">{p.studentEmail}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-5 py-4 text-sm font-medium text-slate-700 dark:text-slate-200">
      ...
      {activeTab === 'plans' && (
        <div className="space-y-5">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Subscription Plans</h3>
              ...
              >
                ...
                <div className="p-5 pb-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {p.isActive ? (
                          ...
                          <span className="text-[10px] font-bold text-slate-400 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded">OFF</span>
                        )}
                      </div>
                  ...
                  <div className="flex items-end gap-2 mb-4">
                    ...
                    {p.originalPrice && (
                      <span className="text-sm text-slate-400 line-through mb-1">₹{p.originalPrice}</span>
                    )}
                    <span className="text-xs text-slate-500 dark:text-slate-400 mb-1">/ {p.durationDays} days</span>
                  </div>

                  {/* Meta */}
                  <div className="flex items-center gap-3 text-[11px] text-slate-400 dark:text-slate-500 mb-4">
                    <span className="font-mono bg-slate-100 dark:bg-slate-800/50 px-2 py-0.5 rounded-md">{p.id}</span>
                ...
                <div className="px-5 pb-5 pt-0">
                  <div className="border-t border-slate-100 dark:border-slate-800/50 pt-4">
                    <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2.5">Features</p>
                    <ul className="space-y-2">
                      {p.features.map((f, idx) => (
                        <li key={idx} className="flex items-center gap-2.5 text-xs text-slate-600 dark:text-slate-300">
          ...
          >
            ...
            <form onSubmit={handleGrantPro} className="space-y-4">
              ...
              <div className="flex items-center gap-3 p-4 rounded-xl bg-slate-50 dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700/50">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-bold text-slate-900 dark:text-white truncate">{grantModalStudent.fullName}</p>
                  <p className="text-xs text-slate-500">{grantModalStudent.email}</p>
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1.5">Select Plan</label>
                <div className="relative">
                  <select
                    value={grantPlanId}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1.5">Duration (Days)</label>
                <input
                  type="number"
                  min={1}
                  required
          ...
          >
            ...
            <form onSubmit={handleSavePlan} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1.5">Plan ID *</label>
                  <input
                    type="text"
                    required
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1.5">Plan Title *</label>
                  <input
                    type="text"
                    required
                    value={planTitle}
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1.5">Price (₹) *</label>
                  <input
                    type="number"
                    min={0}
                    required
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1.5">Orig. Price (₹)</label>
                  <input
                    type="number"
                    min={0}
                    value={planOrigPrice}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1.5">Days *</label>
                  <input
                    type="number"
                    min={1}
                    required
              ...
              <div>
                <label className="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1.5">Features (one per line)</label>
                <textarea
                  rows={4}
              ...
              <label className="flex items-center gap-2.5 cursor-pointer select-none">
                ...
                <span className="text-sm text-slate-600 dark:text-slate-300 font-medium">Plan is active & visible</span>
              </label>
