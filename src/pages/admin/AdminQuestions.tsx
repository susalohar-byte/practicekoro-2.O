...
export const AdminQuestions: React.FC = () => {
  ...
  const handleSubmitQuestion = async (e: React.FormEvent) => {
    ...
    if (!optionA.trim() || !optionB.trim() || !optionC.trim() || !optionD.trim())
      return setFormError('All 4 options (A, B, C, D) are required.');
    ...
    try {
      if (!editingQuestion && ownerType !== 'subject') {
        ...
        if (!res.success)
          return setFormError(res.error || 'Failed to upload question to the test.');
  ...
  const handleExecuteCsvImport = async () => {
    ...
    try {
      ...
      setImportNotice({
        type: 'success',
        text: `Successfully imported ${res.successCount} questions into Question Bank!${res.errorCount > 0 ? ` (${res.errorCount} invalid rows were skipped)` : ''}`,
      });
      await loadData();
    } catch (err) {
      ...
      <div className="rounded-2xl border border-indigo-300/40 bg-indigo-50/70 dark:bg-indigo-500/5 p-4 sm:p-5 shadow-sm">
        <div className="flex items-center justify-between gap-3 pb-3 mb-3 border-b border-indigo-200/50 dark:border-indigo-500/20">
          <div className="flex items-center gap-2.5">
            <span className="w-8 h-8 rounded-xl bg-indigo-600 text-white flex items-center justify-center text-sm">
              💡
            </span>
            <div>
      ...
      <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
          <div className="sm:col-span-2 md:col-span-3 lg:col-span-1 relative">
            ...
            <input
              type="text"
              placeholder="Search question text or short notes..."
              value={searchTerm}
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800/80 pb-3">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="w-7 h-7 rounded-lg bg-purple-500/10 border border-purple-500/20 text-purple-400 font-mono font-bold text-xs flex items-center justify-center">
                    #{idx + 1}
                  </span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 text-xs">
                {(['A', 'B', 'C', 'D'] as const).map((opt) => {
                  ...
                  return (
                    <div
                      key={opt}
                      className={`p-3 rounded-xl border ${q.correctOption === opt ? 'bg-emerald-500/10 border-emerald-500/40 text-emerald-300 font-bold' : 'bg-slate-900/60 border-slate-800 text-slate-300'}`}
                    >
                      <span className="inline-flex w-6 h-6 items-center justify-center rounded-md bg-slate-800 mr-2 font-mono opacity-80">
                        {opt}
                      </span>
                      {value}
                    </div>
            ...
            <form onSubmit={handleSubmitQuestion} className="mt-4 space-y-4">
              ...
              {editingQuestion && (
                <>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        ...
                        className={`py-2 rounded-xl font-bold text-xs border ${sourceType === 'topic' ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-slate-950 border-slate-800 text-slate-400'}`}
                      >
                        📚 Topic Bank
                      </button>
                      <button
                        ...
                        className={`py-2 rounded-xl font-bold text-xs border ${sourceType === 'pyq' ? 'bg-purple-600 text-white border-purple-500' : 'bg-slate-950 border-slate-800 text-slate-400'}`}
                      >
                        📜 PYQ Paper
                      </button>
                    </div>
                  ...
                  {sourceType === 'pyq' && (
                    <div className="p-4 rounded-xl bg-purple-500/5 border border-purple-500/20 grid grid-cols-1 sm:grid-cols-4 gap-3">
                ...
                <div className="grid grid-cols-4 gap-3">
                  {(['A', 'B', 'C', 'D'] as const).map((opt) => (
                    <button
                      ...
                      className={`py-2 rounded-xl font-bold text-xs border ${correctOption === opt ? 'bg-emerald-600 text-white border-emerald-500' : 'bg-slate-950 border-slate-800 text-slate-400'}`}
                    >
                      Option {opt}
                    </button>
                  ))}
              ...
              <div className="rounded-2xl border border-indigo-300/30 bg-indigo-50/70 dark:bg-indigo-500/5 p-4 sm:p-5 space-y-3">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-indigo-500/30 text-xs text-white placeholder:text-slate-600 focus:outline-none focus:border-indigo-400"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-1.5 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-200"
                  >
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`py-2 rounded-xl font-bold text-xs border ${importFormat === 'csv' ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-slate-950 border-slate-800 text-slate-400'}`}
                  >
                    📄 CSV (headers)
                  </button>
                  <button
                    ...
                    className={`py-2 rounded-xl font-bold text-xs border ${importFormat === 'text' ? 'bg-purple-600 text-white border-purple-500' : 'bg-slate-950 border-slate-800 text-slate-400'}`}
                  >
                    📝 Formatted Text
                  </button>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {importFormat === 'csv' && (
                    <input
                      ...
                      className="text-xs text-slate-400 file:mr-2 file:py-1 file:px-2.5 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-indigo-600 file:text-white"
                    />
                  )}
                </div>
                <textarea
                    ...
                    <table className="w-full text-left text-xs text-slate-300">
                      <thead className="bg-slate-950 text-slate-400 text-[10px] uppercase font-bold border-b border-slate-800">
                        <tr>
                          ...
                          <th className="p-2.5">Short Notes</th>
                        ...
                        {csvParseResult.parsedRows.slice(0, 8).map((r) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="p-2.5 max-w-xs truncate">{r.data.questionText}</td>
                              <td className="p-2.5">{r.data.correctOption}</td>
                              <td className="p-2.5">{count ? `${count}/5 bullets` : 'Missing'}</td>
