...
export const AdminQuestions: React.FC = () => {
  ...
  const handleSubmitQuestion = async (e: React.FormEvent) => {
    ...
    const normalizedExplanationBengali = normalizeAndValidateExplanation(explanationBengali, 'Bengali explanation');
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">Standardized repository of bilingual questions, verified answer keys, and pedagogical explanations.</p>
        </div>
      ...
      <div className="space-y-3">
        {isLoading ? <div className="p-12 text-center text-slate-400 text-xs">Loading questions bank...</div> : questions.length === 0 ? <div className="p-12 text-center rounded-2xl bg-slate-950 border border-slate-800 space-y-3"><FileQuestion className="w-10 h-10 text-slate-600 mx-auto" /><p className="text-sm font-bold text-white">No questions found matching filter.</p><Button size="sm" className="bg-indigo-600 hover:bg-indigo-700 text-xs font-bold" onClick={openCreateModal}>Add First Question</Button></div> : questions.map((q, idx) => (
          <div key={q.id} className="p-5 rounded-2xl bg-slate-950 border border-slate-800 hover:border-slate-700 transition-all space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800/80 pb-3">
              <div className="flex flex-wrap items-center gap-2">
                <span className="w-6 h-6 rounded-md bg-purple-500/10 border border-purple-500/20 text-purple-400 font-mono font-bold text-xs flex items-center justify-center">#{idx + 1}</span>
            ...
            <div className="space-y-1"><p className="font-bold text-white text-sm leading-relaxed">{q.questionText}</p>{q.questionBengaliText && <p className="text-slate-300 text-xs font-medium leading-relaxed">{q.questionBengaliText}</p>}</div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 text-xs">{(['A','B','C','D'] as const).map((opt) => { const value = q[`option${opt}` as const]; return <div key={opt} className={`p-2.5 rounded-xl border ${q.correctOption === opt ? 'bg-emerald-500/10 border-emerald-500/40 text-emerald-300 font-bold' : 'bg-slate-900/60 border-slate-800 text-slate-300'}`}><span className="font-mono mr-1.5 opacity-60">{opt}:</span> {value}</div>; })}</div>
            {(q.explanation || q.explanationBengali) && <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800/80 text-xs space-y-2 text-slate-400"><span className="font-bold text-indigo-400 uppercase text-[10px] tracking-wider block">Verified Explanation:</span>{q.explanation && <div className="whitespace-pre-line text-slate-300 leading-relaxed">{normalizeExplanationBullets(q.explanation)}</div>}{q.explanationBengali && <div className="whitespace-pre-line text-slate-400 italic leading-relaxed">{normalizeExplanationBullets(q.explanationBengali)}</div>}</div>}
          </div>
        ))}
      </div>
      ...
      {isModalOpen && <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm overflow-y-auto"><div className="w-full max-w-2xl rounded-2xl bg-slate-900 border border-slate-800 p-6 shadow-2xl my-8"><div className="flex items-center justify-between pb-4 border-b border-slate-800"><h3 className="text-base font-bold text-white flex items-center gap-2"><FileQuestion className="w-4 h-4 text-purple-400" />{editingQuestion ? 'Edit Question Details' : 'Add Question to Bank'}</h3><button type="button" onClick={() => setIsModalOpen(false)} className="text-slate-400 hover:text-white"><X className="w-5 h-5" /></button></div><form onSubmit={handleSubmitQuestion} className="mt-4 space-y-4">{formError && <div className="p-3 rounded-lg bg-rose-500/10 border border-rose-500/20 text-xs text-rose-400">{formError}</div>}
        ...
        <div className="rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-4 space-y-4"><div><p className="text-xs font-black text-white">Explanation / Notes</p><p className="text-[11px] text-slate-400 mt-1">Write 2-5 separate bullet points. Every point must be important and directly related to the question, correct answer, or confusing option/distractor. Use facts that actually help an aspirant clear the doubt.</p></div><div><div className="flex items-center justify-between mb-1"><label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Explanation (English)</label><span className="text-[10px] text-slate-500">2-5 bullets when provided</span></div><textarea rows={7} placeholder={'• Why the correct answer is correct\n• Important fact from the question\n• Difference from a confusing option\n• Exam-relevant memory point'} value={explanation} onChange={(e) => setExplanation(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white placeholder:text-slate-600" /></div><div><div className="flex items-center justify-between mb-1"><label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Explanation (Bengali)</label><span className="text-[10px] text-slate-500">2-5 bullets when provided</span></div><textarea rows={7} placeholder={'• সঠিক উত্তর কেন সঠিক\n• প্রশ্নের গুরুত্বপূর্ণ তথ্য\n• বিভ্রান্তিকর option-এর পার্থক্য\n• পরীক্ষার জন্য মনে রাখার মতো তথ্য'} value={explanationBengali} onChange={(e) => setExplanationBengali(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white placeholder:text-slate-600" /></div></div>
