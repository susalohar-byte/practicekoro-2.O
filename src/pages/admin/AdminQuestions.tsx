...
export const AdminQuestions: React.FC = () => {
  ...
  const handleSubmitQuestion = async (e: React.FormEvent) => {
    ...
    if (normalizedExplanation === null) return;
    const normalizedExplanationBengali = normalizeAndValidateExplanation(
      explanationBengali,
      'Bengali explanation'
    );
    if (normalizedExplanationBengali === null) return;
    ...
    const sharedFields = {
      ...
      explanationBengali: normalizedExplanationBengali || undefined,
          ...
          questions.map((q, idx) => (
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 text-xs">
                <div className={`p-2.5 rounded-xl border ${q.correctOption === 'A' ? 'bg-emerald-500/10 border-emerald-500/40 text-emerald-300 font-bold' : 'bg-slate-900/60 border-slate-800 text-slate-300'}`}>
                  <span className="font-mono mr-1.5 opacity-60">A:</span> {q.optionA}
                </div>
                <div className={`p-2.5 rounded-xl border ${q.correctOption === 'B' ? 'bg-emerald-500/10 border-emerald-500/40 text-emerald-300 font-bold' : 'bg-slate-900/60 border-slate-800 text-slate-300'}`}>
                  <span className="font-mono mr-1.5 opacity-60">B:</span> {q.optionB}
                </div>
                <div className={`p-2.5 rounded-xl border ${q.correctOption === 'C' ? 'bg-emerald-500/10 border-emerald-500/40 text-emerald-300 font-bold' : 'bg-slate-900/60 border-slate-800 text-slate-300'}`}>
                  <span className="font-mono mr-1.5 opacity-60">C:</span> {q.optionC}
                </div>
                <div className={`p-2.5 rounded-xl border ${q.correctOption === 'D' ? 'bg-emerald-500/10 border-emerald-500/40 text-emerald-300 font-bold' : 'bg-slate-900/60 border-slate-800 text-slate-300'}`}>
              ...
              {(q.explanation || q.explanationBengali) && (
                <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800/80 text-xs space-y-2 text-slate-400">
                  <span className="font-bold text-indigo-400 uppercase text-[10px] tracking-wider block">
                    Verified Explanation:
                  </span>
                  {q.explanation && (
                    <div className="space-y-1 whitespace-pre-line text-slate-300">
                      {normalizeExplanationBullets(q.explanation)}
                    </div>
                  )}
                  {q.explanationBengali && (
                    <div className="space-y-1 whitespace-pre-line text-slate-400 italic">
                      {normalizeExplanationBullets(q.explanationBengali)}
                    </div>
                  )}
                </div>
              )}
      ...
      {totalQuestions > PAGE_SIZE && (
        <div className="flex items-center justify-between px-4 py-3 rounded-2xl bg-slate-950 border border-slate-800">
          <span className="text-xs text-slate-400 font-semibold">
            Showing <span className="text-slate-200 font-mono">{(currentPage - 1) * PAGE_SIZE + 1}–{Math.min(currentPage * PAGE_SIZE, totalQuestions)}</span> of <span className="text-purple-400 font-mono">{totalQuestions}</span> questions
          </span>
          <div className="flex items-center gap-2">
            <Button size="sm" variant="outline" disabled={currentPage <= 1} onClick={() => setCurrentPage((p) => Math.max(1, p - 1))} className="border-slate-700 text-xs">← Prev</Button>
            <span className="text-xs text-slate-300 font-mono px-2">Page {currentPage} / {totalPages}</span>
            <Button size="sm" variant="outline" disabled={currentPage >= totalPages} onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))} className="border-slate-700 text-xs">Next →</Button>
          </div>
        </div>
      )}
            ...
            <form onSubmit={handleSubmitQuestion} className="mt-4 space-y-4">
                  ...
                  {ownerType === 'subject' && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <div>
                        ...
                        <select value={subjectId} onChange={(e) => { setSubjectId(e.target.value); setChapterId(''); }} className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-200 focus:outline-none focus:border-indigo-500">
                          <option value="">None / General</option>
                          {subjects.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">Topic</label>
                        <select value={chapterId} onChange={(e) => setChapterId(e.target.value)} disabled={!subjectId} className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-200 focus:outline-none focus:border-indigo-500 disabled:opacity-50">
                          <option value="">None / General</option>
              ...
              {editingQuestion && (
                <>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">Subject (Optional)</label>
                      <select value={subjectId} onChange={(e) => { setSubjectId(e.target.value); setChapterId(''); }} className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-200 focus:outline-none focus:border-indigo-500">
                        <option value="">None / General</option>
                        {subjects.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}
                      </select>
                    ...
                    <div>
                      <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">Topic / Chapter (Optional)</label>
                      <select value={chapterId} onChange={(e) => setChapterId(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-200 focus:outline-none focus:border-indigo-500">
                        <option value="">None / General</option>
                        {modalChapters.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                      </select>
                    </div>
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">Question Origin / Source</label>
                    <div className="grid grid-cols-2 gap-2">
                      <button type="button" onClick={() => setSourceType('topic')} className={`py-2 rounded-xl font-bold text-xs border transition-all ${sourceType === 'topic' ? 'bg-indigo-600 text-white border-indigo-500 shadow-md shadow-indigo-600/30' : 'bg-slate-950 border-slate-800 text-slate-400 hover:text-white'}`}>📚 Topic Bank</button>
                      <button type="button" onClick={() => setSourceType('pyq')} className={`py-2 rounded-xl font-bold text-xs border transition-all ${sourceType === 'pyq' ? 'bg-purple-600 text-white border-purple-500 shadow-md shadow-purple-600/30' : 'bg-slate-950 border-slate-800 text-slate-400 hover:text-white'}`}>📜 PYQ Paper</button>
                    </div>
                  </div>
                  ...
                  {sourceType === 'pyq' && (
                    <div className="p-4 rounded-xl bg-purple-500/5 border border-purple-500/20 space-y-3">
                      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                        <div className="sm:col-span-2"><label className="block text-[11px] font-semibold text-slate-300 mb-1">Exam Name</label><input type="text" placeholder="e.g. WBCS Prelims, KP Constable" value={sourceExam} onChange={(e) => setSourceExam(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-950 border border-slate-800 text-xs text-white" /></div>
                        <div><label className="block text-[11px] font-semibold text-slate-300 mb-1">Year</label><input type="number" placeholder="e.g. 2024" value={sourceYear} onChange={(e) => setSourceYear(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-950 border border-slate-800 text-xs text-white" /></div>
                        <div><label className="block text-[11px] font-semibold text-slate-300 mb-1">Shift / Session</label><input type="text" placeholder="e.g. Shift 1" value={sourceShift} onChange={(e) => setSourceShift(e.target.value)} className="w-full px-3 py-2 rounded-lg bg-slate-950 border border-slate-800 text-xs text-white" /></div>
                      </div>
                    </div>
                  )}
                </>
              ...
              {!editingQuestion && ownerType === 'full_mock' && (
                <div className="p-4 rounded-xl bg-emerald-500/5 border border-emerald-500/20 grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div><label className="block text-[11px] font-semibold text-slate-300 mb-1">Exam Name</label><select value={ownerExamId} onChange={(e) => { setOwnerExamId(e.target.value); setOwnerTestId(''); }} className="w-full px-3 py-2 rounded-lg bg-slate-950 border border-slate-800 text-xs text-white"><option value="">— Select Exam —</option>{ownerExams.map((ex) => <option key={ex.id} value={ex.id}>{ex.title}</option>)}</select></div>
                  <div><label className="block text-[11px] font-semibold text-slate-300 mb-1">Full Mock Test</label><select value={ownerTestId} onChange={(e) => setOwnerTestId(e.target.value)} disabled={!ownerExamId} className="w-full px-3 py-2 rounded-lg bg-slate-950 border border-slate-800 text-xs text-white disabled:opacity-50"><option value="">— Select Full Mock Test —</option>{ownerExamTests.map((t) => <option key={t.id} value={t.id}>{t.title}</option>)}</select></div>
                </div>
              )}
              ...
              {!editingQuestion && ownerType === 'pyq' && (
                <div className="p-4 rounded-xl bg-purple-500/5 border border-purple-500/20 grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div><label className="block text-[11px] font-semibold text-slate-300 mb-1">Exam Name</label><select value={ownerExamId} onChange={(e) => { setOwnerExamId(e.target.value); setOwnerYear(''); setOwnerTestId(''); }} className="w-full px-3 py-2 rounded-lg bg-slate-950 border border-slate-800 text-xs text-white"><option value="">— Select Exam —</option>{ownerExams.map((ex) => <option key={ex.id} value={ex.id}>{ex.title}</option>)}</select></div>
                  <div><label className="block text-[11px] font-semibold text-slate-300 mb-1">Year</label><select value={ownerYear} onChange={(e) => { setOwnerYear(e.target.value); setOwnerTestId(''); }} disabled={!ownerExamId || ownerYears.length === 0} className="w-full px-3 py-2 rounded-lg bg-slate-950 border border-slate-800 text-xs text-white disabled:opacity-50"><option value="">— Select Year —</option>{ownerYears.map((y) => <option key={y} value={y}>{y}</option>)}</select></div>
                  <div><label className="block text-[11px] font-semibold text-slate-300 mb-1">Paper / Shift</label><select value={ownerTestId} onChange={(e) => setOwnerTestId(e.target.value)} disabled={!ownerExamId} className="w-full px-3 py-2 rounded-lg bg-slate-950 border border-slate-800 text-xs text-white disabled:opacity-50"><option value="">— Select Paper / Shift —</option>{ownerPaperTests.map((t) => <option key={t.id} value={t.id}>{t.paperName || t.title}{t.shift ? ` • ${t.shift}` : ''}</option>)}</select></div>
                </div>
              )}
              ...
              <div><label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">Question Text (English) *</label><textarea required rows={2} placeholder="e.g. Who excavated the archaeological ruins of Harappa in 1921?" value={questionText} onChange={(e) => setQuestionText(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white" /></div>
              <div><label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">Question Text (Bengali)</label><textarea rows={2} placeholder="e.g. ১৯২১ সালে হরপ্পা প্রত্নক্ষেত্রটি কে আবিষ্কার করেছিলেন?" value={questionBengaliText} onChange={(e) => setQuestionBengaliText(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white" /></div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {(
                  ...
                  <div key={key}><label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">Option {key} *</label><input type="text" required placeholder={`Option ${key} content`} value={value} onChange={(e) => setter(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white" /></div>
                ))}
              </div>

              <div><label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-2">Correct Answer Key *</label><div className="grid grid-cols-4 gap-3">{(['A', 'B', 'C', 'D'] as const).map((opt) => <button key={opt} type="button" onClick={() => setCorrectOption(opt)} className={`py-2 rounded-xl font-bold text-xs border transition-all ${correctOption === opt ? 'bg-emerald-600 text-white border-emerald-500 shadow-md shadow-emerald-600/30' : 'bg-slate-950 border-slate-800 text-slate-400 hover:text-white'}`}>Option {opt}</button>)}</div></div>

              <div className="rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-4 space-y-4">
                <div>
                  <p className="text-xs font-black text-white">Explanation / Notes</p>
                  <p className="text-[11px] text-slate-400 mt-1">Use 2-5 separate bullet points. Every point must be important and directly related to the question, correct answer, or a confusing option/distractor. The goal is to clear an aspirant's doubt, not add generic filler.</p>
                </div>
                <div>
                  <div className="flex items-center justify-between mb-1"><label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Explanation (English)</label><span className="text-[10px] text-slate-500">2-5 bullets when provided</span></div>
                  <textarea rows={6} placeholder={'• State why the correct answer is correct.\n• Add an important fact directly connected to the question.\n• Clarify the most confusing option/distractor.\n• Add one exam-relevant memory point.'} value={explanation} onChange={(e) => setExplanation(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white placeholder:text-slate-600" />
                </div>
                <div>
                  <div className="flex items-center justify-between mb-1"><label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">Explanation (Bengali)</label><span className="text-[10px] text-slate-500">2-5 bullets when provided</span></div>
                  <textarea rows={6} placeholder={'• সঠিক উত্তর কেন সঠিক তা লিখুন।\n• প্রশ্নের সঙ্গে সরাসরি সম্পর্কিত গুরুত্বপূর্ণ তথ্য দিন।\n• বিভ্রান্তিকর option/distractor-এর পার্থক্য পরিষ্কার করুন।\n• পরীক্ষায় মনে রাখার মতো গুরুত্বপূর্ণ তথ্য দিন।'} value={explanationBengali} onChange={(e) => setExplanationBengali(e.target.value)} className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white placeholder:text-slate-600" />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">Default Marks</label><input type="number" step="0.5" value={defaultMarks} onChange={(e) => setDefaultMarks(parseFloat(e.target.value) || 1)} className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white" /></div>
                <div><label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-1">Negative Mark</label><input type="number" step="0.05" value={defaultNegativeMarks} onChange={(e) => setDefaultNegativeMarks(parseFloat(e.target.value) || 0.25)} className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-white" /></div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
          <div className="w-full max-w-4xl rounded-2xl bg-slate-900 border border-slate-800 p-6 shadow-2xl my-6 max-h-[90vh] flex flex-col">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800 shrink-0">
              <div>
                <h3 className="text-base font-bold text-white flex items-center gap-2"><Upload className="w-4 h-4 text-indigo-400" />Bulk Questions CSV Import</h3>
                <p className="text-xs text-slate-400 mt-0.5">Import multiple questions with explanation fields kept in structured 2-5 bullet format.</p>
              </div>
            ...
            <div className="py-4 space-y-4 overflow-y-auto flex-1">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div><label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Fallback Subject</label><select value={csvDefaultSubject} onChange={(e) => setCsvDefaultSubject(e.target.value)} className="w-full px-3 py-1.5 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-200"><option value="">None</option>{subjects.map((s) => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
                <div><label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Fallback Chapter</label><select value={csvDefaultChapter} onChange={(e) => setCsvDefaultChapter(e.target.value)} className="w-full px-3 py-1.5 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-200"><option value="">None</option>{chapters.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => { setImportFormat('csv'); setCsvParseResult(null); }} className={`py-2 rounded-xl font-bold text-xs border ${importFormat === 'csv' ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-slate-950 border-slate-800 text-slate-400 hover:text-white'}`}>📄 CSV (headers)</button>
                  <button type="button" onClick={() => { setImportFormat('text'); setCsvParseResult(null); }} className={`py-2 rounded-xl font-bold text-xs border ${importFormat === 'text' ? 'bg-purple-600 text-white border-purple-500' : 'bg-slate-950 border-slate-800 text-slate-400 hover:text-white'}`}>📝 Formatted Text (study material)</button>
                </div>
                <p className="text-[11px] text-slate-400 mt-1.5">When explanation is provided, it must contain 2-5 separate bullet points. Each point should clear a doubt using question/correct-answer/option-specific information.</p>
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider">{importFormat === 'text' ? 'Paste Formatted Questions (blank line between questions)' : 'Paste CSV Data or Choose File'}</label>
                  {importFormat === 'csv' && <input type="file" accept=".csv" onChange={handleFileUpload} className="text-xs text-slate-400 file:mr-2 file:py-1 file:px-2.5 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-indigo-600 file:text-white hover:file:bg-indigo-700 cursor-pointer" />}
                </div>
                <textarea
                  rows={importFormat === 'text' ? 12 : 6}
                  placeholder={importFormat === 'text' ? `1. প্রশ্ন?\n(a) Option A\n(b) Option B\n(c) Option C\n(d) Option D\nসঠিক উত্তর: (b)\n\nExplanation:\n- কেন সঠিক\n- প্রশ্নের গুরুত্বপূর্ণ তথ্য\n- বিভ্রান্তিকর option-এর পার্থক্য\n- পরীক্ষার memory point` : 'CSV headers should include explanation and/or explanation_bengali. Use 2-5 bullet points inside the cell.'}
                  value={csvContent}
                ...
                <div className="space-y-3">
                  ...
                  <div className="border border-slate-800 rounded-xl overflow-hidden">
                    <table className="w-full text-left text-xs text-slate-300">
                      <thead className="bg-slate-950 text-slate-400 text-[10px] uppercase font-bold border-b border-slate-800"><tr><th className="p-2.5">Row</th><th className="p-2.5">Status</th><th className="p-2.5">Question Text</th><th className="p-2.5">Key</th><th className="p-2.5">Explanation</th><th className="p-2.5">Errors</th></tr></thead>
                      <tbody className="divide-y divide-slate-800/60 font-mono text-[11px]">
                        {csvParseResult.parsedRows.slice(0, 8).map((r) => {
                          const explanationBullets = normalizeExplanationBullets(r.data.explanation).split('\n').filter(Boolean).length;
                          return (
                            <tr key={r.rowNumber} className={r.isValid ? 'bg-slate-900/40' : 'bg-rose-950/20'}>
                              ...
                              <td className="p-2.5 font-sans font-medium line-clamp-1 max-w-xs">{r.data.questionText || '<Missing>'}</td>
                              <td className="p-2.5 text-indigo-400 font-bold">{r.data.correctOption}</td>
                              <td className="p-2.5 text-slate-300">{explanationBullets ? `${explanationBullets}/5 bullets` : 'Not provided'}</td>
                              <td className="p-2.5 text-rose-400 font-sans text-[10px]">{r.errors.join('; ') || '—'}</td>
                            </tr>
                          );
                        })}
                      </tbody>
              ...
              {importNotice && <div className={`p-3 rounded-xl text-xs ${importNotice.type === 'success' ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400' : 'bg-rose-500/10 border border-rose-500/20 text-rose-400'}`}>{importNotice.text}</div>}
            </div>
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800 shrink-0">
              <Button type="button" variant="outline" size="sm" onClick={() => setIsCsvModalOpen(false)} className="border-slate-700 text-xs">Close</Button>
              <Button type="button" size="sm" onClick={handleExecuteCsvImport} disabled={!csvParseResult || csvParseResult.validCount === 0 || isImporting} className="bg-indigo-600 hover:bg-indigo-700 text-xs font-bold">{isImporting ? 'Importing Questions...' : `Import ${csvParseResult?.validCount ?? 0} Valid Questions`}</Button>
            </div>
          </div>
        </div>
      )}
