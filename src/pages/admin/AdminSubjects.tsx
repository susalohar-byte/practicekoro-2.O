...
export const AdminSubjects: React.FC = () => {
  ...
  const activeSubjectTotalQuestions = selectedSubject
    ...
    : 0;
  const activeSubjectActiveTopics = selectedSubject
    ? chapters.filter((c) => c.subjectId === selectedSubject.id && c.isActive).length
    : 0;
  ...
  const inputClass =
    'w-full px-3 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 transition';
      ...
      <div className="rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950/40 p-5 sm:p-7 shadow-xl shadow-black/10">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-xl sm:text-2xl font-black text-white">Subjects & Topics</h1>
              ...
              <p className="text-sm text-slate-400 mt-1 max-w-2xl">
                এক জায়গা থেকে subject, topic এবং question count সহজে manage করুন।
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            <Button
              size="sm"
              className="bg-indigo-600 hover:bg-indigo-500 font-bold shadow-lg shadow-indigo-600/20"
            ...
            {selectedSubject && (
              <Button
                size="sm"
                variant="outline"
                className="border-emerald-500/30 text-emerald-300 hover:bg-emerald-500/10 font-bold"
                leftIcon={<Plus className="w-4 h-4" />}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-6">
          <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
            <p className="text-[11px] text-slate-500 font-semibold">Subjects</p>
            <p className="text-2xl font-black text-white mt-1">{subjects.length}</p>
          </div>
          <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
            <p className="text-[11px] text-slate-500 font-semibold">Topics</p>
            <p className="text-2xl font-black text-white mt-1">{chapters.length}</p>
          </div>
          <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
            <p className="text-[11px] text-slate-500 font-semibold">Questions</p>
            <p className="text-2xl font-black text-white mt-1">{questions.length}</p>
          </div>
          <div className="rounded-2xl bg-white/5 border border-white/10 p-4">
            <p className="text-[11px] text-slate-500 font-semibold">Active Subjects</p>
          ...
          <div className={`${cardClass} overflow-hidden`}>
            <div className="p-4 border-b border-slate-800 bg-slate-900/60">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h2 className="text-sm font-black text-white">Subjects</h2>
                  <p className="text-[11px] text-slate-500 mt-0.5">
                    Choose a subject to view topics
                  </p>
                </div>
                <span className="px-2 py-1 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-[10px] font-bold">
                  {subjects.length}
                </span>
              </div>
            ...
            <div className="p-2 max-h-[560px] overflow-y-auto">
              {isLoading ? (
                ...
                <div className="p-8 text-center">
                  ...
                  <p className="text-sm font-bold text-slate-300 mt-2">No subjects found</p>
                </div>
              ) : (
                filteredSubjects.map((sub) => {
                  const selected = selectedSubject?.id === sub.id;
                  const topics = subjectTopicCounts[sub.id] || 0;
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left rounded-2xl p-3.5 mb-2 border transition-all ${selected ? 'bg-indigo-600/10 border-indigo-500/40 shadow-md shadow-indigo-950/20' : 'border-transparent hover:bg-slate-900 hover:border-slate-800'}`}
                    >
                      <div className="flex items-start gap-3">
                        <div
                          className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${selected ? 'bg-indigo-600 text-white' : 'bg-slate-900 text-slate-400 border border-slate-800'}`}
                        >
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <ChevronRight
                              className={`w-4 h-4 shrink-0 ${selected ? 'text-indigo-300' : 'text-slate-600'}`}
                            />
                          </div>
                          <div className="flex items-center gap-2 flex-wrap mt-2">
                            <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-slate-900 border border-slate-800 text-[10px] text-slate-300">
                            ...
                            <span
                              className={`ml-auto inline-flex items-center gap-1 text-[10px] font-semibold ${sub.isActive ? 'text-emerald-400' : 'text-slate-500'}`}
                            >
                              <span
                                className={`w-1.5 h-1.5 rounded-full ${sub.isActive ? 'bg-emerald-400' : 'bg-slate-500'}`}
                              />
                      ...
                      <div className="flex items-center justify-end gap-1 mt-3 pt-2 border-t border-white/5">
                        <span
                          ...
                          className="p-1.5 rounded-lg text-slate-500 hover:text-emerald-400 hover:bg-slate-800 cursor-pointer"
        ...
        <section className="xl:col-span-8">
          {selectedSubject ? (
              ...
              <div className="p-5 sm:p-6 border-b border-slate-800 bg-gradient-to-r from-indigo-950/40 via-slate-950 to-slate-950">
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <h2 className="text-2xl font-black text-white mt-1">{selectedSubject.name}</h2>
                    <p className="text-sm text-slate-400 mt-1 max-w-2xl">
                      {selectedSubject.description || 'No description added yet.'}
                    </p>
                  </div>
                  <div className="grid grid-cols-3 gap-2 shrink-0">
                    <div className="rounded-xl bg-slate-900 border border-slate-800 px-3 py-2.5 min-w-[78px] text-center">
                      <p className="text-lg font-black text-indigo-300">
                        {subjectTopicCounts[selectedSubject.id] || 0}
                      </p>
                      <p className="text-[9px] text-slate-500 font-bold uppercase">Topics</p>
                    </div>
                    <div className="rounded-xl bg-slate-900 border border-slate-800 px-3 py-2.5 min-w-[86px] text-center">
                      <p className="text-lg font-black text-blue-300">
                        {activeSubjectActiveTopics}
                      </p>
                      <p className="text-[9px] text-slate-500 font-bold uppercase">Active</p>
                    </div>
                    <div className="rounded-xl bg-slate-900 border border-slate-800 px-3 py-2.5 min-w-[90px] text-center">
                      <p className="text-lg font-black text-emerald-300">
                        {activeSubjectTotalQuestions}
                      </p>
                      <p className="text-[9px] text-slate-500 font-bold uppercase">Questions</p>
                    </div>
                  </div>
                </div>
              </div>
              <div className="p-4 border-b border-slate-800 bg-slate-900/30 flex flex-col md:flex-row gap-3 md:items-center md:justify-between">
                ...
                <div className="flex items-center gap-1 p-1 rounded-xl bg-slate-950 border border-slate-800 w-fit">
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-[11px] font-bold ${topicFilterStatus === 'all' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}
                  >
                    All
                  </button>
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-[11px] font-bold ${topicFilterStatus === 'active' ? 'bg-emerald-600 text-white' : 'text-slate-400 hover:text-white'}`}
                  >
                    Active
                  </button>
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-[11px] font-bold ${topicFilterStatus === 'inactive' ? 'bg-slate-700 text-white' : 'text-slate-400 hover:text-white'}`}
                  >
                    Inactive
                  </button>
                </div>
              ...
              <div className="p-4 space-y-2.5">
                {filteredTopics.length === 0 ? (
                  <div className="rounded-2xl border border-dashed border-slate-800 bg-slate-900/30 p-10 text-center">
                    ...
                    <p className="text-sm font-bold text-white mt-2">No topics found</p>
                    <p className="text-xs text-slate-500 mt-1 mb-4">
                      Add a topic to organise this subject.
                    </p>
                    <Button
                      size="sm"
                  ...
                  filteredTopics.map((topic, index) => {
                    ...
                    return (
                      <div
                        key={topic.id}
                        className="rounded-2xl border border-slate-800/90 bg-slate-900/40 hover:bg-slate-900 hover:border-slate-700 transition-all p-4"
                      >
                        <div className="flex flex-col md:flex-row md:items-center gap-4">
                          <div className="flex items-start gap-3 min-w-0 flex-1">
                            <div className="w-8 h-8 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 flex items-center justify-center text-[11px] font-black shrink-0">
                              {index + 1}
                            </div>
                            <div className="min-w-0">
                              <div className="flex items-center gap-2 flex-wrap">
                                ...
                                <span
                                  className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${topic.isActive ? 'bg-emerald-500/10 text-emerald-300 border border-emerald-500/20' : 'bg-slate-800 text-slate-500 border border-slate-700'}`}
                                >
                              ...
                              <div className="flex items-center gap-2 mt-2">
                                <span
                                  className={`px-2.5 py-1 rounded-lg text-[10px] font-bold border ${qCount ? 'bg-indigo-500/10 text-indigo-300 border-indigo-500/20' : 'bg-slate-950 text-slate-500 border-slate-800'}`}
                                >
                                  <HelpCircle className="inline w-3 h-3 mr-1" />
                                  {qCount} questions
                                </span>
                                <span className="text-[10px] text-slate-600">/{topic.slug}</span>
                              </div>
                            </div>
                          </div>
                          <div className="flex items-center gap-1.5 self-end md:self-center">
                            <button
                              ...
                              className="px-3 py-2 rounded-xl bg-indigo-600/10 hover:bg-indigo-600 text-indigo-300 hover:text-white border border-indigo-500/20 text-xs font-bold flex items-center gap-1.5"
                            >
                            ...
                            <button
                              ...
                              className="p-2 rounded-xl text-slate-500 hover:text-emerald-400 hover:bg-slate-800"
                            ...
                            <button
                              ...
                              className="p-2 rounded-xl text-slate-500 hover:text-white hover:bg-slate-800"
                              title="Edit"
                            >
                              <Edit2 className="w-4 h-4" />
                            </button>
            ...
            <div className={`${cardClass} p-10 text-center`}>
              ...
              <p className="text-sm font-bold text-white mt-2">Select a subject</p>
            </div>
          )}
        </section>
      </div>
          ...
          <div className="w-full max-w-md rounded-2xl bg-slate-900 border border-slate-800 p-6 shadow-2xl">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Keep the curriculum clean and easy to navigate.
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSubjectSubmit} className="mt-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <label className="flex items-center gap-2 rounded-xl border border-slate-800 bg-slate-950 px-3 py-2.5 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={subIsActive}
          ...
          <div className="w-full max-w-md rounded-2xl bg-slate-900 border border-slate-800 p-6 shadow-2xl">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-1">
                  Add a clear topic under the selected subject.
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleTopicSubmit} className="mt-5 space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-slate-800">
                ...
                <Button type="submit" size="sm" disabled={isSavingTopic} className="bg-indigo-600">
