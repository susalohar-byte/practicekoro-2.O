...
export const PopularExams: React.FC = () => {
  ...
  return (
    <section
      id="exams"
      className="relative py-16 sm:py-24 bg-slate-50/50 border-t border-b border-slate-100 scroll-mt-24"
    >
      <div
        aria-hidden="true"
        ...
        <Reveal className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isActive
                      ...
                      : 'bg-white text-slate-600 hover:bg-white hover:text-blue-700 hover:border-blue-200 border border-slate-200/80 shadow-xs'
                  )}
                >
                  {cat.label}
                </button>
          ...
          <div className="relative w-full sm:w-64">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-white border border-slate-200 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15 shadow-xs transition-all"
            />
          </div>
        </Reveal>
            ...
            {filteredPopularExams.map((exam, index) => (
              <Reveal key={exam.title} delay={Math.min(index, 7) * 60} className="h-full">
                <div
                  ...
                  className="group relative h-full rounded-2xl bg-white border border-slate-200/90 p-5 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.04)] hover:shadow-xl hover:shadow-blue-500/10 hover:border-blue-300 hover:-translate-y-1 active:scale-[0.98] transition-all duration-300 cursor-pointer flex items-center justify-between gap-3 overflow-hidden"
                >
                  <span
                    aria-hidden="true"
